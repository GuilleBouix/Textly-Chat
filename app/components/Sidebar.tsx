...
export default function Sidebar({
  ...
  // ============================================
  return (
    <aside className="flex h-full w-full flex-col border-r border-zinc-800/80 bg-zinc-950/72 backdrop-blur-md animate-fade-right md:w-80">
      ...
      <div className="p-4 animate-fade-right animate-delay-50">
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-violet-500 py-2.5 text-sm font-bold text-white shadow-lg shadow-violet-500/10 transition-all hover:bg-violet-400"
        >
          <LuUserPlus size={18} />
          Buscar Contactos
        </button>
      ...
      {mostrarTabsMobile && (
        <div className="px-4 pb-3 md:hidden">
          <div className="grid grid-cols-2 rounded-xl border border-zinc-800/80 bg-zinc-900/45 p-1">
            <button
              onClick={() => setTabMobileActiva("contactos")}
              className={`rounded-lg px-3 py-2 text-xs font-bold tracking-wide transition-colors ${
                tabMobileActiva === "contactos"
            ...
            <button
              onClick={() => setTabMobileActiva("solicitudes")}
              className={`rounded-lg px-3 py-2 text-xs font-bold tracking-wide transition-colors ${
                tabMobileActiva === "solicitudes"
                  ? "bg-violet-500 text-white"
                  : "text-zinc-400 hover:text-zinc-200"
              }`}
            >
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-4 animate-fade-right animate-delay-100">
        ...
        {(!mostrarTabsMobile || tabMobileActiva === "solicitudes") && (
          <>
        ...
        {/* Seccion de chats */}
        {(!mostrarTabsMobile || tabMobileActiva === "contactos") && (
          <>
              ...
              {salas.map((sala) => {
                ...
                return (
                  <div
                    ...
                    className={`group relative flex cursor-pointer items-center gap-3 rounded-xl border p-3 transition-all animate-fade-right animate-delay-350 ${
                      idSalaActiva === sala.id
                        ? "border-violet-400/35 bg-zinc-900/85 shadow-[0_0_0_1px_rgba(167,139,250,0.12)] backdrop-blur-sm"
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {unread > 0 && idSalaActiva !== sala.id && (
                          <span
                            ...
                            className="inline-flex min-w-5 items-center justify-center rounded-full bg-violet-500 px-1.5 text-[10px] font-bold text-white"
                          >
                            {unread > 99 ? "99+" : unread}
                          </span>
                        )}
